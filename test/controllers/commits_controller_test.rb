...
class CommitsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create commit" do
    assert_difference('Commit.count') do
      post commits_url, params: { commit: { body: @commit.body, link_id: @commit.link_id, user_id: @commit.user_id } }
    end
  ...
  test "should update commit" do
    patch commit_url(@commit), params: { commit: { body: @commit.body, link_id: @commit.link_id, user_id: @commit.user_id } }
